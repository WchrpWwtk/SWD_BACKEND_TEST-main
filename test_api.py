...
def test_create_student_score():
    data = {"first_name": "new", "last_name": "student", "subject_title": "Math", "score": 0}

    response = requests.post(
        url=f"{host}/student_score/", headers=header, data=json.dumps(data)
    )
    print(
...
def test_personnel_details():
    response = requests.get(
        url=f"{host}/personnel_details/Dorm Palace School", headers=header
    )
    print(
